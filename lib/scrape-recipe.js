/**
 * Recipe importer — extracts structured recipe data either from a URL
 * (scraping) or from raw pasted text (e.g. a recipe generated by another
 * AI like Gemini, or copied from anywhere).
 *
 * URL strategy:
 ...
 *
 * Pasted-text strategy: skips fetching/HTML-stripping entirely and sends
 * the pasted text straight to Claude with the same extraction schema —
 * genuinely lighter and faster than URL scraping, since there's no page
 * bloat (nav, reviews, related-recipe upsells) to wade through.
 *
 ...
 *
 * KNOWN RISK: headless rendering, and the Claude-fallback path on heavy
 * pages, can be tight against Vercel's 10-second function timeout on the
 * free tier — see lib/headless-fetch.js and the tail-trimming logic below.
 */
...
/** Shared Claude call: takes already-prepared plain text + a label describing
 * its source (used in the prompt only), returns the parsed+validated recipe
 * object. Used by both the URL-scraping fallback and the pasted-text import. */
async function extractRecipeFromText(text, sourceLabel) {
  ...
  // Claude returns its own quantity/unit pairs directly (not via
  // parseIngredientLine), so metric conversion needs to be applied here.
...
async function extractWithClaude(html, sourceUrl) {
  ...
  // some sites this tail is as long as the recipe itself, and reading
  // through it just burns time without adding anything useful. Cut it off
  // at the first common "end of recipe" marker, searched for well past the
  // start so an early false match doesn't cut things off prematurely.
  const tailMarkers = [
  ...
  return extractRecipeFromText(text, `URL: ${sourceUrl}\n\nPage text:`);
}

/** Parses a recipe from raw pasted text (e.g. from another AI like Gemini,
 * or copied from anywhere) — no fetching, no HTML stripping, no tail
 * trimming needed since the input is already just the recipe itself. */
async function extractFromPastedText(text) {
  const trimmed = text.trim().slice(0, 15000);
  if (trimmed.length < 20) {
    throw new Error('That text looks too short to be a recipe — paste the full ingredients and steps.');
  }
  const recipe = await extractRecipeFromText(trimmed, 'Pasted recipe text:');
  return { ...recipe, sourceUrl: null };
}
...
/** Saves an already-extracted recipe object (+ its ingredients) into
 * Supabase. Shared by both the URL-scraping and pasted-text import paths. */
...
/** Scrapes a recipe from a URL and saves it. Returns the saved row with its DB id. */
...
/** Parses a recipe from pasted text and saves it. Returns the saved row with its DB id. */
async function importPastedRecipe(text) {
  const recipe = await extractFromPastedText(text);
  return saveRecipe(recipe);
}

/** Overwrites an existing recipe's data with a freshly extracted one — used
 * when re-importing (new URL or new pasted text) into the same recipe
 * instead of creating a duplicate. Nickname is never touched here (it's a
 * user choice, not scraped data). The existing photo is kept unless the new
 * import provides its own image, so a manually-uploaded photo survives a
 * re-import from a source with no image. */
async function updateRecipe(recipeId, recipe) {
  const { supabase } = require('./db');

  const { data: existing, error: fetchError } = await supabase
    .from('recipes').select('image_url').eq('id', recipeId).single();
  if (fetchError) throw new Error(`Recipe not found: ${fetchError.message}`);
  ...
  const { data: updatedRecipe, error: updateError } = await supabase
    ...
    .update({
      ...
      cook_time_minutes: recipe.cookTimeMinutes,
      steps: recipe.steps,
      image_url: recipe.imageUrl || existing.image_url,
      extraction_method: recipe.extractionMethod,
      confidence: recipe.confidence
    })
    .eq('id', recipeId)
  ...
  const { error: deleteIngError } = await supabase
    .from('recipe_ingredients').delete().eq('recipe_id', recipeId);
  if (deleteIngError) throw new Error(`Failed to clear old ingredients: ${deleteIngError.message}`);
  ...
  if (recipe.ingredients.length > 0) {
    ...
    const { error: insertIngError } = await supabase
      .from('recipe_ingredients').insert(ingredientRows);
    if (insertIngError) throw new Error(`Failed to save new ingredients: ${insertIngError.message}`);
  }

  return { ...recipe, id: updatedRecipe.id };
}

/** Scrapes a recipe from a URL and overwrites an existing recipe with it. */
async function scrapeAndUpdateRecipe(url, recipeId) {
  const recipe = await scrapeRecipe(url);
  if (recipe.needsBrowser) return recipe;
  return updateRecipe(recipeId, recipe);
}

/** Parses a recipe from pasted text and overwrites an existing recipe with it. */
async function updatePastedRecipe(text, recipeId) {
  const recipe = await extractFromPastedText(text);
  return updateRecipe(recipeId, recipe);
}

module.exports = {
  scrapeRecipe, scrapeAndSaveRecipe, importPastedRecipe,
  scrapeAndUpdateRecipe, updatePastedRecipe,
  extractJsonLd, normaliseJsonLd, parseIngredientLine
};
